...
class CPOBuffer:
    ...
    def __init__(self, size,
        ...
        self.gamma, self.lam = gamma, lam   # swb：这个gamma和lam到底是什么啊？!
    ...
    def store(self, obs, act, rew, val, cost, cval, logp, pi_info):
        ...
        self.obs_buf[self.ptr] = obs   # swb:这些buffer，都是预先在numpy中开辟好的啊！!!
        self.act_buf[self.ptr] = act
        self.rew_buf[self.ptr] = rew  # swb:算GAE和V都要用到
        self.val_buf[self.ptr] = val  # swb:算GAE要用到
    ...
    def finish_path(self, last_val=0, last_cval=0):
        ...
        rews = np.append(self.rew_buf[path_slice], last_val)  # swb：注意这边reward这里面也用到了那个last-value啊
        vals = np.append(self.val_buf[path_slice], last_val)
        deltas = rews[:-1] + self.gamma * vals[1:] - vals[:-1]  # swb: r + γ*v_{t+1} - v_{t}
        ...
        self.path_start_idx = self.ptr  # swb：然后就把那个位置给更新上去了啊！！！！
    ...
    def get(self):
        ...
        self.cadv_buf -= cadv_mean  # swb:这边对cost没有进行rescale啊！！
