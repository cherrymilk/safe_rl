...
def placeholder(dim=None):
    return tf.placeholder(dtype=tf.float32, shape=combined_shape(None,dim))  # swb:name=None了啊!!
...
def placeholders_from_spaces(*args):
    return [placeholder_from_space(space) for space in args]  # swb:这样返回的是个list啊！
...
"""
# swb: https://zhuanlan.zhihu.com/p/54706353
...
def mlp_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    act_dim = a.shape.as_list()[-1]  # swb:获得了一个int，为动作空间的纬度呢。。
    mu = mlp(x, list(hidden_sizes)+[act_dim], activation, output_activation)  # swb:定义了一个神经网络啊！！一共3层数（256，256，2）
    log_std = tf.get_variable(name='log_std', initializer=-0.5*np.ones(act_dim, dtype=np.float32))  # swb：初始化为（0.5, 0.5）
    # swb：log_std的话，这样有正有负，所以好训练啊，要不然这个std原理上来说一定是正数啊！！
    std = tf.exp(log_std)
    pi = mu + tf.random_normal(tf.shape(mu)) * std  # swb：这个就是那个采样噪声啊。。(动作+噪音)
    # swb: log[π(a|s)]
    logp = gaussian_likelihood(a, mu, log_std)  # swb:这个得看一下啊，没有看懂啊。。(均值mu，方差log_std的情况下选a的概率啊)
    # swb: log[π(pi|s)]
    logp_pi = gaussian_likelihood(pi, mu, log_std)  # swb：(均值mu，方差log_std的情况下选pi的概率啊)

    old_mu_ph, old_log_std_ph = placeholders(act_dim, act_dim)
    d_kl = gaussian_kl(mu, log_std, old_mu_ph, old_log_std_ph)  # swb:这个kl是解析的啊！！
    ent = gaussian_entropy(log_std)  # swb:动作的entroy啊
...
def mlp_actor_critic(x, a, hidden_sizes=(64,64), activation=tf.tanh,
    ...
    with tf.variable_scope('vf'):  # swb:一个是value function
        v = tf.squeeze(mlp(x, list(hidden_sizes)+[1], activation, None), axis=1)
        # swb:把输出的v值，从（batch, 1）--> (batch, )
    with tf.variable_scope('vc'):
        vc = tf.squeeze(mlp(x, list(hidden_sizes)+[1], activation, None), axis=1)
        # swb:把输出的vc值，从（batch, 1）--> (batch, )
