...
class PPOAgent(Agent):
    ...
    def __init__(self, clip_ratio=0.2,
        ...
        self.pi_lr = pi_lr  # swb:这个是更新策略网络的学习率啊！！
        self.pi_iters = pi_iters  # swb:策略的迭代次数啊！！
        self.kl_margin = kl_margin  # swb：这个是kl的margin呢。。
        self.params.update(dict(  # swb：这样会把dict加入原先的dict里面去啊！！
            clipped_adv=True,
            first_order=True,
            constrained=False
        ))
    ...
    def update_pi(self, inputs):
        ...
        d_kl = self.training_package['d_kl']  # swb:这个是已经求过平均的值了啊！！
        target_kl = self.training_package['target_kl']  #swb:目标的kl散度啊，不能大于这个值啊！！
        # Run the update
        for i in range(self.pi_iters):  # swb:d_kl不能和kl的名字一样啊！！！！！！
            _, kl = self.sess.run([train_pi, d_kl], feed_dict=inputs)
            kl = mpi_avg(kl)  # swb:会在各个线程上同步，然后算出一个平均值呢！！
            if kl > self.kl_margin * target_kl:  # swb:感觉这边好像没有几次就退出了啊！！
                self.logger.log('Early stopping at step %d due to reaching max kl.' % i+1)
                break  # swb:如果kl散度超标了，那么就会中途停止更新了啊！
        self.logger.store(StopIter=i+1)
...
class CPOAgent(TrustRegionAgent):
    ...
    def update_pi(self, inputs):
        flat_g = self.training_package['flat_g']  # swb: \hat{g}  pi_loss对于神经网络的导数啊！！
        flat_b = self.training_package['flat_b']  # swb: \hat{b}  surr_cost对于神经网络的导数！！
        v_ph = self.training_package['v_ph']  # swb:这个是placeholder啊！！
        hvp = self.training_package['hvp']   # swb: \hat{H} 这个得到的就是H*v
        get_pi_params = self.training_package['get_pi_params']  # swb:得到神经网络的参数啊！！
        set_pi_params = self.training_package['set_pi_params']  # swb:对神经网络进行赋值操作!!
        pi_loss = self.training_package['pi_loss']
        surr_cost = self.training_package['surr_cost']  # swb: \hat{c}
        d_kl = self.training_package['d_kl']
        target_kl = self.training_package['target_kl']
        ...
        if self.learn_margin:  # swb:这个是false啊！！
        ...
        self.margin = mpi_avg(self.margin)  # swb：这个margin是什么狗屎东西！？
        ...
        if optim_case in [3, 4]:
            lam = np.sqrt(q / (2 * target_kl))  # swb:最大化reward，同时满足kl散度的约束！！
            nu = 0
            ...
            lam = 0
            nu = np.sqrt(2 * target_kl / (s + EPS))  # swb:这个公式的意义是说，如果不满足约束的话，就会最小化cost，同时满足散度的约束啊！！
        ...
        def set_and_eval(step):  # swb:这个是在做 backtracking line-search
        ...
        for j in range(self.backtrack_iters):
            ...
            self.logger.log('%d \tkl %.3f \tsurr_cost_new %.3f' % ((j+1), kl, surr_cost_new), 'blue')
            if (kl <= target_kl and   # swb:如果是optim_case=0的场景，那么就单纯的减少cost较多情况下，那么action-pair的概率啊！！
                    (pi_l_new <= pi_l_old if optim_case > 1 else True) and
                     surr_cost_new - surr_cost_old <= max(-c, 0)):
                self.logger.log('Accepting new params at step %d of line search.' % (j+1))
                self.logger.store(BacktrackIters=(j+1))
                break
